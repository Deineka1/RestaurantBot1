...
b17 = KeyboardButton('Меню клиента')
kb_client = ReplyKeyboardMarkup(resize_keyboard=True).row(b4, b5, b6).add(b17)

#Кнопки меню На месте
b7 = KeyboardButton('Официант')
b8 = KeyboardButton('Меню')
b9 = KeyboardButton('Счет')
kb_nameste = ReplyKeyboardMarkup(resize_keyboard=True).row(b7, b8, b9).add(b17)

#Кнопки меню С собой
b10 = KeyboardButton('*Меню')
kb_ssoboy = ReplyKeyboardMarkup(resize_keyboard=True).row(b10).add(b17)

#Кнопки меню Доставка
b11 = KeyboardButton('*Меню*')
b12 = KeyboardButton('Посмотреть заказ')
kb_dostavka = ReplyKeyboardMarkup(resize_keyboard=True).row(b11, b12).add(b17)
